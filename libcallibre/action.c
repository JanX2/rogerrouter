...
struct action *action_create(const gchar *name, const gchar *description, const gchar *exec, guchar flags)
{
	...
	settings_path = g_strconcat("/org/tabos/callibre/profile/", profile_get_active()->name, "/", name, "/", NULL);
	action->settings = g_settings_new_with_path(CALLIBRE_SCHEME_PROFILE_ACTION, settings_path);
...
static void action_load(struct profile *profile, const gchar *name)
{
	...
	settings_path = g_strconcat("/org/tabos/callibre/profile/", profile->name, "/", name, "/", NULL);
