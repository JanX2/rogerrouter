...
#include <libcallibre/audio.h>
#include <libcallibre/gstring.h>

#define CALLIBRE_TYPE_PULSEAUDIO_PLUGIN (callibre_pulseaudio_plugin_get_type())
#define CALLIBRE_PULSEAUDIO_PLUGIN(o) (G_TYPE_CHECK_INSTANCE_CAST((o), CALLIBRE_TYPE_PULSEAUDIO_PLUGIN, CallibrePulseAudioPlugin))

typedef struct {
	guint id;
} CallibrePulseAudioPluginPrivate;

CALLIBRE_PLUGIN_REGISTER(CALLIBRE_TYPE_PULSEAUDIO_PLUGIN, CallibrePulseAudioPlugin, callibre_pulseaudio_plugin)
...
{
	//CallibrePulseAudioPlugin *pulseaudio_plugin = CALLIBRE_PULSEAUDIO_PLUGIN(plugin);
	callibre_audio_register(&pulse_audio);
}
