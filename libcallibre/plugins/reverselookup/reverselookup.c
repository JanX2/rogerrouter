...
#define CALLIBRE_TYPE_REVERSE_LOOKUP_PLUGIN (callibre_reverse_lookup_plugin_get_type ())
#define CALLIBRE_REVERSE_LOOKUP_PLUGIN(o) (G_TYPE_CHECK_INSTANCE_CAST((o), CALLIBRE_TYPE_REVERSE_LOOKUP_PLUGIN, CallibreReverseLookupPlugin))

typedef struct {
	guint signal_id;
	GHashTable *table;
} CallibreReverseLookupPluginPrivate;

CALLIBRE_PLUGIN_REGISTER(CALLIBRE_TYPE_REVERSE_LOOKUP_PLUGIN, CallibreReverseLookupPlugin, callibre_reverse_lookup_plugin)
...
{
	CallibreReverseLookupPlugin *reverselookup_plugin = user_data;
	struct contact *rl_contact;
	gchar *name;
	gchar *street;
	gchar *zip;
...
{
	CallibreReverseLookupPlugin *reverselookup_plugin = CALLIBRE_REVERSE_LOOKUP_PLUGIN(plugin);

	reverselookup_plugin->priv->table = g_hash_table_new(g_str_hash, g_str_equal);
	table = g_hash_table_new(g_str_hash, g_str_equal);

	callibre_lookup_register(reverse_lookup);
