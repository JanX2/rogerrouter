...
struct profile *profile_new(const gchar *name, const gchar *host, const gchar *user, const gchar *password)
{
	...
	settings_path = g_strconcat("/org/tabos/callibre/profile/", name, "/", NULL);
	profile->settings = g_settings_new_with_path(CALLIBRE_SCHEME_PROFILE, settings_path);
...
{
	settings = g_settings_new(CALLIBRE_SCHEME);
