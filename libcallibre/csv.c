...
/**
 * \TODO: Split general csv function and callibre journal functions
 */
...
#include <libcallibre/plugins/fritzbox/csv.h>

/** This is our private header, not the one used by the router! */
#define CALLIBRE_JOURNAL_HEADER "Typ;Datum;Name;Rufnummer;Nebenstelle;Eigene Rufnummer;Dauer"
...
gboolean csv_save_journal(GSList *journal)
{
	...
	dir = g_build_filename(g_get_user_data_dir(), "callibre", profile->name, NULL);
	...
	fprintf(file, "sep=;\n");
	fprintf(file, CALLIBRE_JOURNAL_HEADER);
	fprintf(file, "\n");
...
{
	list = csv_parse_data(data, CALLIBRE_JOURNAL_HEADER, csv_parse_callibre, list);
...
GSList *csv_load_journal(GSList *journal)
{
	...
	file_name = g_build_filename(g_get_user_data_dir(), "callibre", profile->name, "journal.csv", NULL);
